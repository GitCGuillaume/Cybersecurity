 ...
 */
fn get_url_header(client: &Client, path: &str) -> Result<Response, Error> {
    let res: Result<Response, Error> = client.get(path).send();

    res
}


pub fn url_helper(options: &parse_flags::parse::OptionUser, cli: &Client,
    ...
    for (k, v) in hmap_url.clone() {
        ...
        let res: Result<Response, Error> = get_url_header(cli, &k);
        match res {
            Ok(r) => {
                ...
                if !res {
                    let res_txt: Result<String, Error> = r.text();

                    match res_txt {
                        Ok(txt) => {
                            find_input = document::parse_doc(&options, cli, &txt, hmap_url);
                        },
                        Err(e) => {
                    ...
                    let _ = img::process_image(r, options);
                }
            },
            Err(e) => {
                eprintln!("Error: {e}");
